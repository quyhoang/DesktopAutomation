...
KaiwAIData := import(KaiwAIFunctionality, KaiwAIDefaultSplittedData)
YukarilinkData := import(YukarilinkFunctionality, YukarilinkDefaultSplittedData)
ClipGenieData := import(ClipGenieFunctionality, ClipGenieDefaultSplittedData)
HayanabiData := import(HayanabiFunctionality, HayanabiDefaultSplittedData)

import(functionality, defaultData)
{
	%functionality%Data := ["", "", "", "", "", "", "", "", "", ""]
	; Check if the folder exists
	fFolder := A_ScriptDir . "\" . functionality . "Folder"
	if not FileExist(fFolder)
	{
		; Create the folder
		FileCreateDir, %fFolder%
		
		; Notify the user
		TrayTip, Folder Created, Folder created: %functionality%Folder.`nData is default initialized., 1, 17
		return defaultData
	}
	
	noData := true ; check if the files contain no data. If so, default initialize the variable
	loop, 10
	{
		index := A_Index - 1
		filePath := A_ScriptDir . "\" . functionality . "Folder\" . index . ".txt"
		if FileExist(filePath)
		{
			...
			%functionality%Data[index] := fileContent
		}
	}

	if noData
	{
		TrayTip, Default initialized, %functionality% data is default initialized., 1, 17
		return defaultData
	}
	else
	{
		;TrayTip, Import Completed, Imported entries in %functionality%, 1, 5
